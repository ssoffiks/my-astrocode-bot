export function parseBirthDate(input) {
  ...
  if (!match) {
    return { ok: false, error: 'Введи дату в формате ДД.ММ.ГГГГ. Например: 14.08.1997' };
  }
  ...
  if (year < 1900 || year > new Date().getFullYear()) {
    return { ok: false, error: 'Проверь год рождения. Он должен быть в формате ГГГГ.' };
  }
...
export function parseBirthTime(input) {
  ...
  if (!match) {
    return { ok: false, error: 'Введи время в формате ЧЧ:ММ. Например: 08:45' };
  }
